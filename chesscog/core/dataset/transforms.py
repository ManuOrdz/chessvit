...
def build_mask_transforms(cfg: CN, mode: Datasets) -> typing.Callable:
    """
    Build transforms for segmentation masks (solo transformaciones geométricas).

    Args:
        cfg (CN): Configuración global
        mode (Datasets): TRAIN o VAL

    Returns:
        Callable: función de transformación para máscaras
    """
    transforms = cfg.DATASET.TRANSFORMS
    t = []

    # 🔹 Recorte central (si se usa en imágenes)
    if transforms.CENTER_CROP:
        t.append(T.CenterCrop(transforms.CENTER_CROP))
